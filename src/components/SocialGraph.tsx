...
// Helper to get profile image from follow
...
// Helper to get name from follow
...
// Helper to get address from follow
...
export function SocialGraph({ address }: SocialGraphProps) {
  ...
  const { followerCount, followingCount, isLoading: loadingCount } = useFollowCount({
    address: targetAddress?.toLowerCase() || '',
  });

  // Get followers (people following this address) with infinite scroll
  const {
    follows: followers,
  ...
  // Get following (people this address follows) with infinite scroll
  const {
    follows: following,
  ...
  // Auto-fetch all pages for followers
  useEffect(() => {
    if (activeTab === 'followers' && hasMoreFollowers && !loadingMoreFollowers) {
      fetchMoreFollowers();
    }
  }, [activeTab, hasMoreFollowers, loadingMoreFollowers, fetchMoreFollowers]);

  // Auto-fetch all pages for following
  useEffect(() => {
    if (activeTab === 'following' && hasMoreFollowing && !loadingMoreFollowing) {
      fetchMoreFollowing();
    }
  }, [activeTab, hasMoreFollowing, loadingMoreFollowing, fetchMoreFollowing]);

  // === Mutual follow detection (client-side Set-based) ===
  // Build sets for O(1) lookup - no API calls needed!
  ...
  // Mutual follow: address is in both sets
  ...
  const isLoading = loadingCount || loadingFollowers || loadingFollowing;
  ...
  return (
    <div style={styles.card}>
      ...
      {showPlaceholder && (
        <p style={styles.empty}>🔌 Connect your wallet to view social graph</p>
      )}
